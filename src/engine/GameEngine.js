...
import { initializeTurnOrder, advanceTurn, getActiveEntity, markPlayerActed } from './TurnManager.js';
import { applyEffect, tickEffects, hasEffect } from './StatusEffects.js';
import { placeTrap, checkTrapTrigger, deactivateTrap, getAvailableTrapTypes } from './TrapSystem.js';
import { processBossHunt, checkWildlifeAttack } from './WildlifeSystem.js';
import { spawnFlora, relocateFlora, searchFlora } from './FloraSystem.js';
import { resolveRetreat } from './RetreatSystem.js';
...
export function createInitialState(blueprint) {
  ...
  const floraState = spawnFlora(blueprint.zones, blueprint.systems?.flora);
...
export function gameReducer(state, action) {
  switch (action.type) {
    ...
    case ActionTypes.START_GAME: {
      ...
      const turnState = initializeTurnOrder(playerList);
      const bossState = createBossState(blueprint);
      const floraState = spawnFlora(blueprint.zones, blueprint.systems?.flora);
    ...
    case ActionTypes.PLAYER_ATTACK: {
      ...
      if (result.hit) {
        newState = applyDamageToBoss(newState, result.damage);
        // Track consecutive hits
        const newConsec = player.consecutiveHits + 1;
        const newDamageDealt = (player.damageDealt || 0) + result.damage;
        newState = {
          ...newState,
          players: {
            ...newState.players,
    ...
    case ActionTypes.PLAYER_RETREAT: {
      ...
      const { outcome, newZoneId, narrative, heal } = resolveRetreat(
    ...
    case ActionTypes.BOSS_AOE_ATTACK: {
      ...
      for (const player of alivePlayers) {
        ...
        if (result.hit) {
          newState = applyDamageToPlayer(newState, player.id, result.damage);
          narrativeText += ` ${player.name}: ${result.damage} damage.`;
        }
      }
    ...
    case ActionTypes.RUN_ENVIRONMENT: {
      ...
      if (floraSettings?.enabled) {
        const newFloraState = relocateFlora(floraState, blueprint.zones, floraSettings, round);
      ...
      if (blueprint.systems?.wildlife?.enabled) {
        ...
        const huntResult = processBossHunt(boss.zone, newState, blueprint, huntRoll.raw);
      ...
      if (boss.zone) {
        ...
        const trapResult = checkTrapTrigger(boss.zone, newState, trapRoll.raw);
    ...
    case ActionTypes.ADVANCE_PHASE: {
      ...
      // Advance turn state
      const playerCount = Object.values(state.players).filter(p => p.alive).length;
      const newTurnState = advanceTurn(
        state.turnState || initializeTurnOrder(Object.values(state.players)),
        playerCount
      );
